...
class DatabaseService {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 32) {
      try {
        await db.execute('ALTER TABLE scan_results ADD COLUMN image_path TEXT');
      } catch (_) {}
    }
    if (oldVersion < 38) {
      for (final col in [
  ...
  Future<void> _seed(Database db) async {
    const seeds = [
      ...
      // ── Recipe pantry staples (added for ingredient nutrition coverage) ──
      FoodData(
          label: 'Sugar',
          densityMin: 0.80,
          densityMax: 0.90,
